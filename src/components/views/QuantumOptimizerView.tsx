    ...
    try {
      ...
      setTimeout(() => {
        ...
        try {
          ...
          // graceful fallback if canvas-confetti is not supported
        }
      }, 700);
    } catch (err) {
      console.error('Error running QUBO optimizer:', err);
      setIsSolving(false);
    }
  };
        ...
        <div className="lg:col-span-4 rounded-2xl bg-white border border-slate-200/90 p-5 lg:p-6 shadow-xs space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              <button
                type="button"
                onClick={() => { setAlpha(1.2); setBeta(1.2); setGamma(1.5); setTargetCount(2); }}
              ...
              <button
                type="button"
                onClick={() => { setAlpha(0.7); setBeta(2.2); setGamma(2.5); setTargetCount(1); }}
              ...
              <button
                type="button"
                onClick={() => { setAlpha(2.2); setBeta(0.9); setGamma(1.1); setTargetCount(2); }}
                          ...
                          <tr key={rowIdx} className="hover:bg-slate-50/50">
                            ...
                            {quboResult.quboMatrix.variables.map((_, colIdx) => {
                              ...
                              const val = isDiagonal
                                ? quboResult.quboMatrix.linearCoefficients[rowIdx]
